...
avl_t *avl_insert_rec(avl_t **c_node, avl_t *p_node, avl_t **n_node_ptr, int v_node);
...
avl_t *avl_insert_rec(avl_t **c_node, avl_t *p_node, avl_t **n_node_ptr, int v_node)
{
    ...
    if ((*c_node)->n > v_node)
    {
        (*c_node)->left = avl_insert_rec(&(*c_node)->left, *c_node, n_node_ptr, v_node);
        if ((*c_node)->left == NULL)
            return (NULL);
    }
    else if ((*c_node)->n < v_node)
    {
        (*c_node)->right = avl_insert_rec(&(*c_node)->right, *c_node, n_node_ptr, v_node);
